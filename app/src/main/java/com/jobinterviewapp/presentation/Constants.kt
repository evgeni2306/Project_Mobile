package com.jobinterviewapp.presentation

object Constants {
    const val PARAM_FIELD_OF_ACTIVITY_ID = "fieldOfActivityId"
    const val PARAM_DIRECTION_OF_FIELD_ID = "directionsOfField"
    const val PARAM_TECHNOLOGIES_OF_DIRECTION_ID = "technologiesOfDirection"
    const val PARAM_PROFESSIONS_OF_TECHNOLOGY_ID = "professionsOfTechnology"
    const val PARAM_INTERVIEW_TASK_COUNT = "interviewTaskCount"
    const val ENTRY_NAVIGATION_ANIMATION_DURATION = 300
    const val USER_SETTINGS_FILE_NAME = "user-settings.json"
    const val PARAM_INTERVIEW_ID = "PARAM_INTERVIEW_ID"
}